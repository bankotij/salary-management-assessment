...
export default function EmployeesPage() {
  ...
  return (
    <section className="employees-page">
      ...
      {!loading && !error && employees.length > 0 && <EmployeeTable employees={employees} />}

      <div className="pagination">
        <button type="button" onClick={() => setPage((p) => p - 1)} disabled={loading || !canGoPrevious}>
          Previous
        </button>
        <span className="pagination__info">
          Page {page} of {totalPages} ({total} employees)
        </span>
        <button type="button" onClick={() => setPage((p) => p + 1)} disabled={loading || !canGoNext}>
          Next
        </button>
      </div>
    </section>
